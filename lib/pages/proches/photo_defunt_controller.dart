...
class PhotoDefuntController extends GetxController with StateMixin<List> {
  //
  ...
  getAllProche(String id) async {
    ...
    //
    Response response = await requete.getE("media/images?id=$id");
    //
    if (response.isOk) {
  ...
  setSupprimer(String id, idDef) async {
    ...
    //change([], status: RxStatus.loading());
    //
    Response response = await requete.deleteE("media/image?id=$id");
    //
    if (response.isOk) {
      print("body: ${response.statusCode}");
      print("body: ${response.body}");
  ...
  //
  addPhoto(String idDefunt, Uint8List photo) async {
    //
    var url = Uri.parse("${Requete.url}/media/image?id=$idDefunt");
